...
#pragma unroll
  for (int i = 0; i < UNROLL; ++i) { reg[i] = ldg_cg(ldg_ptr + BLOCK * i); }

  int sum = 0;
#pragma unroll
  for (int i = 0; i < UNROLL; ++i) { sum += reg[i]; }
...
int main() {
  ...
  for (int i = 0; i < WARMUP_ITER; ++i) { kernel<BLOCK, UNROLL, N_DATA><<<grid, BLOCK>>>(x, y); }

  cudaEventRecord(start);
  for (int i = 0; i < BENCH_ITER; ++i) { kernel<BLOCK, UNROLL, N_DATA><<<grid, BLOCK>>>(x, y); }
