...
// K: ldA
// N: ldB
template<const int BLOCK_SIZE_M = 128,  // height of block of C that each thread block calculate
         const int BLOCK_SIZE_K = 8,  // width of block of A that each thread block load into shared
                                      // memory
...
__global__ void Sgemm(float* __restrict__ A, float* __restrict__ B, float* __restrict__ C,
  ...
  const int A_TILE_ROW_STRIDE = THREAD_NUM_PER_BLOCK / A_TILE_THREAD_PER_ROW;  // 128
  const int B_TILE_ROW_STRIDE = THREAD_NUM_PER_BLOCK / B_TILE_THREAD_PER_ROW;  // 4
...
#pragma unroll
  for (int i = 0; i < BLOCK_SIZE_M; i += A_TILE_ROW_STRIDE) {
    ...
    FETCH_FLOAT4(ldg_a_reg[ldg_index]) = FETCH_FLOAT4(A[OFFSET(A_TILE_ROW_START + i,  // row
                                                               A_TILE_COL,            // col
                                                               K)]);
...
#pragma unroll
    for (int thread_x = 0; thread_x < THREAD_SIZE_X; thread_x += 4) {
      FETCH_FLOAT4(C[OFFSET(BLOCK_SIZE_M * by + ty * THREAD_SIZE_Y + thread_y,
...
int main(int argc, char** argv) {
  ...
  // generate A
  for (int i = 0; i < M * K; i++) { h_A[i] = i / 13; }

  // generate B
  for (int i = 0; i < K * N; i++) { h_B[i] = i % 13; }
  ...
  printf("My gemm Performance= %.2f GFlop/s, Time= %.3f msec, Size= %.0f Ops,\n", gigaFlops[0],
         msecPerMatrixMul[0], flopsPerMatrixMul);
  ...
  checkCudaErrors(cudaEventRecord(start));
  for (int run = 0; run < nIter; run++) {
    cublasSgemm(blas_handle, CUBLAS_OP_T, CUBLAS_OP_T, M, N, K, &alpha, d_A, K, d_B, N, &beta, d_C,
                N);
  }
  checkCudaErrors(cudaEventRecord(stop));
  checkCudaErrors(cudaEventSynchronize(stop));
  ...
  printf("CuBlas Performance= %.2f GFlop/s, Time= %.3f msec, Size= %.0f Ops,\n", gigaFlops[1],
         msecPerMatrixMul[1], flopsPerMatrixMul);
